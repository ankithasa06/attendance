...
export default function Layout({ children, user }: LayoutProps) {
  ...
  // Set to true whenever you are ready to enable the Leave feature in production
  const ENABLE_LEAVES_FEATURE = false;
  ...
  const navItems = [
    ...
    ...(ENABLE_LEAVES_FEATURE ? [{ name: 'Leave Requests', path: '/leaves', icon: CalendarClock, adminOnly: false }] : []),
    { name: 'Employees', path: '/employees', icon: Users, adminOnly: true },
    { name: 'Locations', path: '/locations', icon: MapPin, adminOnly: true },
    ...(ENABLE_LEAVES_FEATURE ? [{ name: 'Leave Management', path: '/admin-leaves', icon: CalendarClock, adminOnly: true }] : []),
...
function SettingsModal({ isOpen, onClose, user }: { isOpen: boolean, onClose: () => void, user: AuthUser }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Error updating credentials', description: err.message || 'Something went wrong', variant: 'destructive' });
    } finally {
